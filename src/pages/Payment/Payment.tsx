...
export const Payment = () => {
  ...
  return (
    <Block className="z-50 w-full">
      {isLoading && !hasError && (
        <div
          className="inset-0 flex flex-col items-center justify-center z-20"
      ...
      {hasError && (
        <div
          className="inset-0 flex flex-col items-center justify-center  z-20 p-6"
          style={{ top: "52px", height: "calc(100% - 52px)" }}
        >
      ...
      <iframe
        ref={iframeRef}
        src={url}
        className={`w-full rounded-3xl h-[calc(100vh-52px)] border-0 transition-opacity duration-300 ${isLoading ? "opacity-0" : "opacity-100"}`}
        title="Payment"
        allow="payment *"
